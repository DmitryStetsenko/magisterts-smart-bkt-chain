...
Цей документ містить детальний опис архітектурного дизайну, структури репозиторію, обраних технологій та інфраструктури для реалізації децентралізованої системи адаптивного навчання та автоматизованого підтвердження компетенцій **«Smart-BKT-Chain» (AlgoProof.org / KnowLedger.org)**.
...
graph TB
    ...
    subgraph Backend ["Серверний рівень (Backend)"]
        ...
        BKT["BKT Engine (Bayesian Logic)"]
        LLM["LLM Judge (Pre-processing)"]
        Prisma["Prisma ORM"]
    end
...
### 2.1. Концепція наскрізної типізації (End-to-End Type Safety)

Для запобігання помилкам узгодженості даних на різних рівнях системи, монорепозиторій реалізує наскрізну типізацію за трьома основними векторами:
...
*   **Штучний інтелект та математика:**
    *   **BKT-модуль:** Окрема сервісна служба на NestJS, що оперує матрицями ймовірностей для розрахунку $P(L)$. Для подолання проблеми **«Холодного старту» (Cold Start)** впроваджується обов'язковий вхідний діагностичний тест для розрахунку індивідуального стартового $P(L_0)$.
    *   **LLM Judge (Гібридний підхід):** Для уникнення високих затримок (latency) та витрат на API, перевірка коду будується каскадно: спочатку виконуються локальні швидкі Unit-тести / статичний аналіз AST на бекенді, і лише у разі потреби семантичного аналізу нестандартних рішень або формування розширеного текстового фідбеку підключається LLM.
